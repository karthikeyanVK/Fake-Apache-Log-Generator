...
parser.add_argument("--output", "-o", dest='output_type', help="Write to a Log file, a gzip file or to STDOUT", choices=['LOG','GZ','CONSOLE','KAFKA'] )
...
while (flag):
    ...
    if output_type == "KAFKA":
        ack = producer.send(topicName, b'%s - - [%s %s] "%s %s HTTP/1.0" %s %s\n' % (ip,dt,tz,vrb,uri,resp,byt))
        metadata = ack.get()
